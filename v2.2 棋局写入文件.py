...
class Checkerboard:
	...
	def get_diagonalline(self,places,color):
		cb=self.checkerboard
		you=color
		opponent=1-you
		n=6		
	...
	# 找出可以放棋的位置
		...
		self.clear_board()
		places=[]	
		places=self.get_row(places,color)
		places=self.get_col(places,color)
		places=self.get_diagonalline(places,color)
	...
	# alive=1 alive;0 you lose;-1 ai lose
		...
		if alive==9:
			...
			print('BLACK CHESS ROUND',file=f)
		elif alive==0:
			print('WHITE CHESS ROUND')
			print('WHITE CHESS ROUND',file=f)
		print()
		print('',file=f)	
		...
		print('black chess:',self.bcnt,'  white chess:',self.wcnt,file=f)
...
class Game:
	...
	def make_2_players(self):		
		...
		if not mode:
			...
			level=input('Choose AI\'s level(0-random, 1-minimax with alpha beta prune): ')
			
			human_color=int(color)	
			ai_color=1-human_color
			level=int(level)
			
			print('>>> Human: ',['white','black'][human_color],file=f)
			print('>>> AI: ',['white','black'][ai_color],', ',['random', 'minimax with alpha beta prune'][level],file=f)
			print('='*60,file=f)
